...
adListFile=./adlists.list
adListDefault=./adlists.default
whitelistScript="./advanced/Scripts/unlist.sh"
whitelistFile=./whitelist.txt
blacklistFile=./blacklist.txt

#Source the setupVars from install script for the IP
piholeInterface=eth0
IPv4_address=172.16.1.203/24
IPv6_address=
piholeDNS1=8.8.8.8
piholeDNS2=8.8.4.4
...
basename=pihole
piholeDir=/Users/gt/dev/pi-hole-modified
adList=${piholeDir}/lightness.json
...
gravity_hostFormat() {
	...
	echo "::: Formatting domains into a JSON file..."
	echo "[" > ${piholeDir}/${accretionDisc}
    cat ${piholeDir}/${eventHorizon} | awk '{sub(/\r$/,""); print "{\"action\":{\"type\":\"block\"},\"trigger\":{\"url-filter\":\".*\",\"if-domain\":[\""$0"\"]}},"}' >> ${piholeDir}/${accretionDisc}
	echo "{\"action\":{\"type\":\"block\"},\"trigger\":{\"url-filter\":\".*\",\"if-domain\":[\"awhateverdomain.net\"]}}]" >> ${piholeDir}/${accretionDisc}
#	echo "[{\"trigger\":{\"url-filter\":\".*\",\"if-domain\":[" > ${piholeDir}/${accretionDisc}
#    cat ${piholeDir}/${eventHorizon} | awk '{sub(/\r$/,""); print "\""$0"\","}' >> ${piholeDir}/${accretionDisc}
#	echo "\"animpossibledomainnamejusttoshutdowntheJSONparser.com\"" >> ${piholeDir}/${accretionDisc}
#	echo "]},\"action\":{\"type\":\"block\"}}]" >> ${piholeDir}/${accretionDisc}
...
gravity_advanced() {
	...
	cat ${piholeDir}/${matterAndLight} | awk -F'#' '{print $1}' | awk '($1 !~ /^#/) { if (NF>1) {print $2} else {print $1}}' | gsed -nr -e 's/\.{2,}/./g' -e '/\./p' >  ${piholeDir}/${supernova}
	echo " done!"
